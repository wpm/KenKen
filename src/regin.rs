...
pub fn regin(domains: Vec<BTreeSet<Value>>) -> Vec<BTreeSet<Value>> {
    let n = domains.len();
    if n == 0 {
        return domains;
    }
    ...
    let mut result: Vec<BTreeSet<Value>> = domains;
