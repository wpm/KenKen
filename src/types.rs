...
impl fmt::Display for Puzzle {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "{}x{} KenKen ({} cages)", self.latin_square.n, self.latin_square.n, self.cages.len())?;
        for cage in &self.cages {
            writeln!(f, "  {}", cage)?;
        }
        Ok(())
...
mod tests {
    ...
    fn make_3x3_puzzle() -> Puzzle {
        ...
        Puzzle {
            cages: vec![
                Cage { cells: vec![(0, 0), (1, 0)], op: Operation::Add(5) },
                Cage { cells: vec![(0, 1), (0, 2)], op: Operation::Add(4) },
                Cage { cells: vec![(1, 1)],          op: Operation::Given(2) },
                Cage { cells: vec![(1, 2), (2, 2)], op: Operation::Mul(2) },
                Cage { cells: vec![(2, 0), (2, 1)], op: Operation::Sub(2) },
            ],
            latin_square: ls,
        }
    }
