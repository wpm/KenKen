//! `Delta` is an overlay of candidate-value sets used to transform a
//! [`crate::Puzzle`]'s grid via [`crate::Puzzle::narrow`] (per-cell
//! intersection) or [`crate::Puzzle::widen`] (per-cell union).

use crate::{
    grid::Grid,
    types::{Cell, Error, Fill, Index},
};

/// An overlay of candidate-value sets, shaped like a [`Grid`] but distinguished
/// from it by the type system: a [`Delta`] is a transform, not a state.
#[must_use]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Delta(Grid);

impl Delta {
    /// Identity delta for an n×n grid: every cell is `Fill::full(n)`.
    ///
    /// # Errors
    ...
    pub fn set(self, cell: Cell, values: Fill) -> Self {
...
mod tests {
    ...
    fn identity_every_cell_is_full() {
        ...
        for row in 0..3 {
            for column in 0..3 {
                assert_eq!(d.get(Cell::new(row, column)).unwrap(), Fill::full(3));
            }
        }
    }
    ...
    fn set_overrides_target_cell() {
        let d = Delta::identity(3)
            .unwrap()
            .set(Cell::new(1, 2), Fill::new([1]));
        assert_eq!(d.get(Cell::new(1, 2)).unwrap(), Fill::new([1]));
    }
    ...
    fn set_leaves_other_cells_unchanged() {
        let d = Delta::identity(3)
            .unwrap()
            .set(Cell::new(1, 2), Fill::new([1]));
        assert_eq!(d.get(Cell::new(0, 0)).unwrap(), Fill::full(3));
    }
    ...
    fn get_returns_full_for_unset_cell_on_identity() {
        ...
        assert_eq!(d.get(Cell::new(0, 1)).unwrap(), Fill::full(2));
    }
}
